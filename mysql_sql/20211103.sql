...
GROUP BY groupName, prodName WITH ROLLUP ORDER BY groupName DESC, prodName DESC;
...
-- 내림 FLOOR()
SELECT addr, FLOOR(avg(height)) AS '평균키' FROM userTbl
GROUP BY addr ORDER BY 평균키 DESC;

-- 지역별 평균키를 구하고 전체 회원의 평균 키를 검색
SELECT addr, ROUND(avg(height)) AS '평균키' FROM userTbl
GROUP BY addr WITH ROLLUP ORDER BY 평균키 DESC;
...
UPDATE buyTbl2 SET 
prodName = '면바지', price = 40, amount = 7, groupName = '의류'
WHERE num = 8;
SELECT * FROM buyTbl2;
...
-- buyTb2에서 아이디가 BBK인 정보 삭제
DELETE FROM buyTbl2 WHERE userID = 'BBK';
SELECT * FROM buyTbl2;


-- commit; 실행된 DML구문을 실행
...
-- table에 삽입된 모든 행의 정보를 삭제하고 commit;
