...
CREATE TABLE IF NOT EXISTS userTbl(
    ...
    mobile1 char(3),
    mobile2 char(8),
    height smallint, -- 적은량의 데이터를 저장하는 실수
    mDate date	-- 년,월,일 저장
);
...
-- prodTbl의 파일이름을 buyTbl로 변경
ALTER TABLE prodTbl rename buyTbl;

show tables;
-- buyTbl 있는 price 속성에 dataType을 INT로 변경 (MODIFY 속성을 변경)
...
-- buyTbl count열 이름을 amount로 변경 (CHANGE 속성이름 변경)
...
SELECT first_name AS '이름', gender AS '성별', hire_date AS '입사일'  -- AS 생략가능
FROM employees;
