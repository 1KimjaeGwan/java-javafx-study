...
SELECT name FROM userTbl WHERE birthYear = 1973;

-- 2. 011 전화번호를 사용하고 서울에 사는 사람의 이름과 주소 키 검색
SELECT name, addr, height FROM userTbl WHERE mobile1 = '011' AND addr = '서울';

-- 3. 경기에 사는 사람 중에 키가 170 이하인 사람 의 정보 검색
SELECT * FROM userTbl WHERE addr = '경기' AND height <= 170;

-- 4. 2013년도에 가입한 사람의 이름과 전화번호 검색
SELECT name, mobile1, mobile2 FROM userTbl WHERE mdate >= '2013-01-01' AND mdate <= '2013-12-31';
SELECT name, mobile1, mobile2 FROM userTbl WHERE mdate BETWEEN 20130101 AND 20131231;
SELECT name, mobile1, mobile2 FROM userTbl WHERE mdate LIKE '2013%';
...
SELECT name,birthYear FROM userTbl WHERE birthYear < 1969;

SELECT name,birthYear FROM userTbl 
	WHERE birthYear < (SELECT birthYear FROM userTbl WHERE name = '윤종신');
...
WHERE userID IN(SELECT userID FROM buyTbl WHERE groupName = '의류'));
-- 174보다 큰사람
SELECT name,height FROM userTbl
...
SELECT userID,count(*) AS '구매횟수' FROM buyTbl GROUP BY userID HAVING 구매횟수 >= 3;
...
SELECT userID,sum(price * amount) AS '구매금액' FROM buyTbl GROUP BY userID HAVING 구매금액 >= 1000 ORDER BY 구매금액 DESC;
