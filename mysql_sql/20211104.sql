...
-- MySQL 내장 함수
-- dual 테이블
-- 테이터가 없고, 함수 계산을 위해 사용하는 가상의 테이블
...
-- 두 시간 사이의 간격(차이)를 계산
-- timestampdiff(년월일,비교할 기준시간, 비교할 기준시간);
...
-- substr 사용
SELECT hiredate FROM emp;
SELECT empno, ename, deptno FROM emp -- 1981-02-20
...
-- SCOTT 제외
SELECT b.ename FROM emp AS a, emp AS b
...
-- 매니저가 아닌 사람 
-- JOIN 조건에서 벗어난 우측 테이블의 행 정보 까지 JOIN 범위에 포함
-- SMITH ALLEN WARD MARTIN TURNER ADAMS MILLER
