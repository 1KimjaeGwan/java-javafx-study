...
-- 기준테이블과 조인 테이블 모두 데이터가 존재해야 조회됨.
...
-- Data를 찾기 위한 데이터
-- 책의 목차에 비유
-- 특정 테이블의 특정 컬러(열)을 지정하여 인덱스를 생성
...
delimiter $$
    ...
    BEGIN
        ...
        WHILE(i < 10) DO
            ...
            INSERT INTO temp_tbl VALUES(str); -- 테이블에 저장
			SET i = i + 1; -- 자리수 증가
        END WHILE; -- while 종료
...
INSERT INTO member_tbl(id,pw,name) VALUES(null, 'pw002','박종혁');
...
DELIMITER $$
    ...
    BEGIN
		DECLARE result VARCHAR(10);	-- 검색결과를 문자열로 저장
        SET result = (
...
SET @result = '';
SELECT @result;
INSERT INTO member_tbl(id,pw,name) VALUES('id003','pw003','예성준');
SELECT @result;
SELECT * FROM member_tbl;
DELETE FROM member_tbl WHERE num = 3;
SELECT @result;

DROP TRIGGER test_trg;
...
-- 출생년도가 1890년 이전에 태어난 사용자로 정보가 등록되면
-- 출생년도를 0으로 등록해서 잘못된 정보를 알려주고
-- 출생년도가 현재시간보다 크면 현재시간으로 등록
use sqlDB;
...
INSERT INTO userTbl VALUES('ZZZ','지지지','1888','평양','051','0000000',182,'2020-12-25');

SELECT * FROM userTbl WHERE userID = 'ZZZ';

INSERT INTO userTbl VALUES('SHS','심현석','2988','개성','071','0000000',164,'2020-12-25');
