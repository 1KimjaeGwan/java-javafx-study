...
ALTER TABLE usertbl MODIFY userID char(8);

DESC userTbl;

-- 데이터 넣기/삽입 INSER INTO ~ VALUES
INSERT INTO userTbl VALUES('HGD','홍길동',1000,'서울','010','000000',176,20130505);

SELECT * FROM userTbl;

INSERT INTO buyTbl(userID,prodName,price,amount) VALUES('HGD','책',30,5);
...
WHERE name = '김경호'; -- 조건절 : 이름 속성이 '김경호'와 일치하는 사용자
...
SELECT * FROM userTbl WHERE birthYear >= 1970 AND birthYear <= 1979;
...
SELECT * FROM userTbl WHERE mDate < '2009-01-01' AND height = 182;
...
-- 김경호 와 키가 같거나 키가 큰사람의 이름과 키를 검색
...
SELECT * FROM userTbl ORDER BY addr ASC, birthYear DESC;

-- 검색 결과 열의 중복되는 값을 제거 - distinct
SELECT addr FROM userTbl ORDER BY addr;
-- 키가 175이상인 사람이 사는 곳
SELECT DISTINCT addr FROM userTbl WHERE height >= 175;
...
SELECT * FROM userTbl ORDER BY mDate DESC LIMIT 6 OFFSET 5;
...
SELECT userID, sum(amount * price) AS '구매금액' FROM buyTbl GROUP BY userID;

-- 상품이름 prodName , 판매개수 amount
-- buyTbl에서 상품별로 몇개나 판매되었는지 상품을 그룹화하여 검색
SELECT prodName, sum(amount) AS 판매개수 FROM buyTbl GROUP BY prodName ORDER BY 판매개수 DESC;
...
SELECT count(*) FROM userTbl WHERE mobile1 IS null;

SELECT count(DISTINCT addr) FROM userTbl;
SELECT DISTINCT addr FROM userTbl;

-- min() , max() - 속성에 저장된 최소값과 최고값을 검색
-- userTbl에서 키가 가장 작은 사람의 이름과 키를 검색
...
height = (SELECT min(height) FROM userTbl) OR
height = (SELECT max(height) FROM userTbl);

-- buyTbl에 등록된 상품들 중에 가장 가격이 낮은 상품과 
-- 가장 가격이 높은 상품의 이름과 가격을 검색
SELECT * FROM buyTbl;
SELECT prodName, price FROM buyTbl WHERE price IN(
(SELECT max(price) FROM buyTbl),
(SELECT min(price) FROM buyTbl) 
) GROUP BY prodName;
SELECT DISTINCT prodName, price FROM buyTbl WHERE 
price = (SELECT max(price) FROM buyTbl) OR
